...
- Follow-up Cubic review on `packages/omo-senpi/plugin/scripts/stage-agent-toolkit.mjs`: `--check` skips only when both generated source and target are absent; partial source/runtime states now fail.
...
- `PATH="/tmp/opencode/bun140/bin:$PATH" OMO_AGENT_TOOLKIT_TARGET=/tmp/opencode/pr6725-missing-agent-toolkit node packages/omo-senpi/scripts/expect-exit.mjs 1 -- node packages/omo-senpi/plugin/scripts/stage-agent-toolkit.mjs --check`
...
- The partial generated-runtime check failed with exit code 1 and `Senpi agent-toolkit runtime is missing: /tmp/opencode/pr6725-missing-agent-toolkit`.
...
- The partial-state and both-missing checks prove `build-extension.mjs --check` cannot pass with only one side of the generated source/runtime pair missing.
